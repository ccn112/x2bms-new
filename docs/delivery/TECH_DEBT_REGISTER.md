...
| T9 | Hard-lock tenant tầng DB — theo ①③ (MySQL): ③ cổng ratchet xong; ① composite FK xong cho notifications + **NHÓM TIỀN 13 quan hệ** (statements/payments/receipts/debts/apartment_wallets/liability_periods → buildings/apartments/billing_periods/payments/residents). DB chặn lai-tenant (1452). ② trigger `payment_allocations` XONG (mig `..._000004`: chặn allocation nối payment↔statement/line lai-tenant, reject SQLSTATE 45000). **CÒN:** lan test MUST_NOT_LEAK ra mọi bề mặt đọc; audit data-leak rộng (IDOR/over-exposure/PII — SECURITY_CONTROLS §5). Postgres RLS: hoãn. Xem `docs/adr/ADR-001-tenant-scope-discipline.md` | `..._000003_add_tenant_composite_fk_money_group.php` | G10 |
