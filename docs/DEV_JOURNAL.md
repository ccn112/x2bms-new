...
## 2026-07-21 — Gom nav thành sub-nav (collapsed) + nhóm "Lưu trữ & Sao lưu"

Nav dài → đặt **navigationGroups collapsed mặc định** cả 3 panel (admin/sa/hq) + gán icon nhóm. Tách 2 màn SA (TenantLifecycleManager/TenantBackupManager) từ 'SaaS Billing' sang nhóm mới **'Lưu trữ & Sao lưu'**. /admin vốn đã ẩn sẵn các trang detail (shouldRegisterNavigation=false) nên chỉ còn "Tổng quan" + 5 nhóm collapsed. Verify: `php -l` sạch, SA nav groups đăng ký đủ (gồm nhóm mới), page render OK.
