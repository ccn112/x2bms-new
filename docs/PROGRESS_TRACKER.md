...
| 03-03 Tạo/Chạy kỳ phí | 🟡 | `Services/Billing/Engine/*` (ChargeDraft, ManagementFeeGenerator, BillingRunner), command `billing:run` | **04/08: có RUNNER (Phase 2 P2.1).** Engine khung + generator THUẦN family `management` (area×giá, half-up, VAT, snapshot), ghi `approval_status=pending` (không tự phát hành), idempotent. Command dry-run/--commit. Test `BillingEngineManagementTest` 3/3. **CÒN:** P2.2 xe · P2.3 điện/nước bậc thang · P2.4 per-use · P2.5 phạt/lãi + đối soát số kế toán import ≥2 kỳ (§5) trước khi bật. Xem `BILLING_FEE_ENGINE_PHASE2_PLAN.md` |
| 03-03bis Nhập khoản phí (kế toán import) | 🟢 | `Pages/BillingChargeImport.php`, `Support/Import/Profiles/BillingChargeImportProfile.php`, `RowNormalizers::money()`, cmd `billing:backfill-fee-family` | **MỚI 31/07 (Phase B1, reference slice — `BILLING_IMPORT_SPEC_20260731.md`).** Trên `StagingImporter` dùng chung: 16 cột (family suy từ `fee_type_code`), tiền số nguyên đồng, tài sản (BKS/đồng hồ) bắt buộc khớp đúng căn — không khớp thì CHẶN, không đoán. Statement luôn sinh `pending` (D1); dòng đã tồn tại re-import thì CẬP NHẬT, không nhân đôi, KHÔNG chạm `paid_amount`. Có "Hoàn tác lô" (chỉ khi còn pending). 9 test `BillingChargeImportTest` (idempotent · số lẻ bị chặn · `518.000`/`518,000` cùng ra 518000 · BKS sai căn bị chặn · đồng hồ trùng loại thiếu cột Tài sản bị chặn · nợ cũ dồn kỳ không gộp · pending không lộ cư dân · total_amount là phép chiếu · rollback pending sạch/published bị từ chối). Backfill `fee_category`→5 family đã chạy trên DB dev (6002/7212 dòng đổi). KHÔNG làm: engine tính phí (Phase C), phát hành (B2), phân bổ (B3) |
| 03-04 Duyệt & Phát hành bảng kê | 🟢 | `Services/Billing/StatementApprovalService.php`, `Pages/StatementList.php`, model `Statement`,`StatementApproval`,`StatementPublishLog` | **XONG 2026-07-31 (Phase B2)**: `approve()`/`reject()`/`publish()` — chỗ DUY NHẤT set `approval_status='published'` (trước đây 0 dòng code). Duyệt ghi `StatementApproval` (trước không có write-path); phát hành ghi `StatementPublishLog` + `published_at`. Nút "Duyệt/Từ chối/Phát hành" thật ở `StatementList` (trước là `<button>` KHÔNG `wire:click`, thuần trang trí). 8 test `StatementApprovalServiceTest`. **Còn nợ**: `StatementList.php:47` hardcode `$today='2026-07-02'`, `:68` sort hash-shuffle, filter/search ở blade chưa nối (`<select>` không `wire:model`) — ngoài phạm vi B2, chưa sửa |
...
| 03-06 Duyệt & phát hành | 🟢 | `StatementApprovalService.php`, `StatementApprovalQueue.php`, `MyWork.php` | **SỬA 2026-07-31 (Phase B2, gate G9)**: 3 đường vòng đã xác minh đều đóng — `MyWork::decide()` loại `statement` nay gọi `StatementApprovalService` (chặn tự duyệt + guard trạng thái, trước là mass-update trần); `StatementApprovalQueue::transitionRuns()` nay lọc trạng thái hợp lệ trước khi chuyển (trước chuyển mù, ví dụ từ chối được cả run đã `published`); `/fila/payments` (`PaymentResource`) đổi CHỈ ĐỌC — bỏ `EditAction`/`RestoreAction`/`ForceDeleteAction`/bulk, xoá luôn form free-text `status`. Lưu ý: `StatementApprovalQueue` vẫn duyệt ở cấp `BillingRun` (batch), KHÔNG cascade xuống `Statement.approval_status` — hai trục vẫn tách biệt, chưa hợp nhất |
| 03-06bis Phân bổ tiền theo dòng phí | 🟢 | `StatementLine::allocationSortKey()`, `ResidentPaymentClaimReviewer::allocateToClaimedStatement()`, `ApartmentWalletService::autoSettleOutstanding()`, cmd `billing:reconcile-statement-balances` | **MỚI 2026-07-31 (Phase B3, D3)**: `payment_allocations.statement_line_id` (cột có sẵn, trước 0 dòng code ghi) nay được ghi thật — chứng từ chuyển khoản cư dân đi qua từng dòng phí theo khoá ưu tiên DÙNG CHUNG (`is_critical` trước, `payment_priority` tăng dần, dòng cũ trước), không tự chọn riêng mỗi nơi. `statements.paid_amount`/`status` nay là PHÉP CHIẾU (`Statement::recomputePaidAmount()`), không ghi tay. Sửa kèm `ApartmentWalletService::autoSettleOutstanding()` (M8 — dead code, 0 caller, sẽ phá bất biến nếu bật nguyên trạng vì ghi `line.paid_amount` mà bỏ qua `statement.paid_amount`) để cùng gọi `recomputePaidAmount()`. Fallback: statement KHÔNG có dòng phí nào (dữ liệu legacy) thì giữ hành vi phẳng cũ ở cấp statement. 20 test mới. Thứ tự ưu tiên theo family + override theo dự án → xem dòng 03-06ter |
| 03-06ter Thứ tự ưu tiên phân bổ (D4-bis) | 🟢 | cmd `billing:backfill-fee-priority`, model `FeeTypePriorityOverride`, `StatementLine::effectivePaymentPriority()`, `Pages/FeePriorityOrder.php` (slug `fees/priority`) | **MỚI 2026-08-01 (Phase B4)**: backfill `fee_types.payment_priority` theo family D4 (QL 100/Nước 200/Điện 300/Xe 400/Khác 900, qua `BillingFamily::defaultPriority()` — không viết logic suy family lần hai). Chạy trên DB dev: 36/39 dòng đổi, 3 đã đúng sẵn, idempotent (rerun lần 2: 0 đổi). Guard ghi đè tay: cột mới `fee_types.payment_priority_locked_at` — NULL thì backfill được ghi đè tự do, khác NULL thì bỏ qua (chưa có UI nào set cột này, backfill hôm nay an toàn ghi đè toàn bộ, cột tồn tại cho tương lai). Override theo dự án: bảng mới `fee_type_priority_overrides` (project_id, fee_type_id, payment_priority — KHÔNG mở rộng `fee_scope_assignments` vì khác mục đích/vòng đời), `StatementLine::allocationSortKey()` gọi `effectivePaymentPriority()` tra override trước, không có thì về mặc định tenant-wide. Cả hai call site B3 (`ResidentPaymentClaimReviewer`, `ApartmentWalletService`) đã eager-load `statement.building` để suy `project_id` không phát sinh N+1. UI kéo-thả `/admin/fees/priority`: thao tác trên override (không đụng `fee_types`), nút "Khởi tạo từ mặc định" (seed override = thứ tự tenant hiện tại) + "Khôi phục mặc định" (xoá override). 4 test mới `FeePaymentPriorityTest` (backfill đúng family, backfill không đụng dòng khoá tay + ổn định khi rerun, override đổi thứ tự CHỈ dự án đó, dự án khác vẫn mặc định). **Chưa làm/để lại**: chưa có UI cấp tenant để tự sửa tay `fee_types.payment_priority` (chỉ backfill mới ghi cột này hiện tại) — không cần cho D4, chỉ ghi nhận |
| 03-07 Công nợ theo căn | 🟡 | `Pages/DebtLedger.php`, model `Debt` | `debts` **không có nguồn tính** — buckets/risk/recovery chỉ seeder, không job tính lại ⇒ số ở đây và HQ DebtAging **không đảm bảo khớp** `statements`. (map dùng `debt_ledgers`; code dùng `Debt`) — B5 (D6, ngăn tiền thừa theo tài sản + màn công nợ theo dịch vụ) CHƯA làm, hết ngân sách phiên sau khi xong B4 |
...
| 07-10 Phân tích hiệu quả | 🟢 | `NotificationAnalyticsService`, page `/admin/notifications/analytics` | Open-rate + phễu giao nhận per-kênh (từ delivery_logs) + chi phí kênh trả phí; scope visibleTo; test service 3/3. CTR click-event chưa track (dùng read-rate proxy) |
...
- **GĐ3 (grants & membership) — XONG 2026-08-01**: bảng mới `community_membership_grants`
  (migration `2026_08_01_100000_create_community_membership_grants.php`) theo đúng
  schema `COMMUNITY_DB_MAPPING.md` §3 (`source_type`
  `resident_relation|manual_join|invitation|system_enrollment`, `source_id` = id
  **quan hệ** resident↔apartment chứ không phải apartment id, unique
  `(membership_id,source_type,source_id)`). Cùng migration: `community_group_members.resident_id`
  chuyển NULLABLE + thêm unique `(community_group_id,user_id)` — cần cho auto-enroll
  X2Living của tier `member` thuần (trước đây bảng bắt buộc phải có `resident_id`,
  không biểu diễn được người chưa có hồ sơ Resident nào).
  `app/Services/Community/MembershipService.php` (mới) là chỗ DUY NHẤT cấp/thu hồi
  membership qua grant — bất biến cốt lõi COM-007: **membership chỉ mất khi hết TOÀN
  BỘ grant active**, không phải hết một grant bất kỳ. `grant()`/`revoke()` là hai hàm
  lõi (transaction + `lockForUpdate`, idempotent); `grantResidentRelation()`/
  `revokeResidentRelation()`, `grantManualJoin()`/`revokeManualJoin()`,
  `enrollPlatformCommunity()` là 3 cặp tiện ích gọi vào đó.
  **Wiring đã làm**: (1) `ResidentApprovalQueue::approve()` (`WEB-02-04` — duyệt cư
  dân) gọi `grantResidentRelation()` ngay sau khi tạo `ResidentApartmentRelation` —
  đây là CHỖ DUY NHẤT trong repo tạo quan hệ căn hộ cho một kích hoạt cư dân mới; (2)
  `BootstrapController::me()` gọi `enrollPlatformCommunity()` đầu hàm — mọi phiên app
  đăng nhập đều chạm `GET me/bootstrap`, chọn đây làm điểm hội tụ enroll duy nhất thay
  vì rải hook ở từng luồng đăng ký (OTP/social/mật khẩu); (3)
  `CommunityController::joinGroup()/leaveGroup()` đổi từ `firstOrCreate`/`delete` trần
  sang gọi `grantManualJoin()`/`revokeManualJoin()` — hệ quả: rời nhóm nay chỉ set
  `left_at` (không xoá cứng dòng `community_group_members` — giữ lịch sử), nên
  `CommunityController::groups()` phải thêm `whereNull('left_at')` vào điều kiện tính
  `joined` (không có thì rời nhóm xong app vẫn thấy đã tham gia — sửa cùng lúc, không
  ai gặp vì trước đây `leaveGroup()` xoá cứng nên "joined" trần không cần điều kiện
  này).
  **Khoảng trống đã biết, cố ý để lại**: repo hiện KHÔNG có luồng "kết thúc quan hệ
  căn hộ" nào — `resident_apartment_relations` không có `end_date`/`status`, migration
  gốc (`2026_06_28_000007_create_residents_and_structure.php`) không có, và không nơi
  nào trong code xoá/archive một quan hệ (đã `grep` toàn repo để xác nhận). Vì vậy
  `revokeResidentRelation()` viết sẵn, có **2 test cách ly** chứng minh đúng hành vi,
  nhưng CHƯA có call site sản xuất nào gọi tới — khi tính năng "chấm dứt hợp đồng/bán
  nhà" được xây (ngoài phạm vi GĐ3), đó là nơi phải gọi hàm này. `ResidentImportProfile`
  (nhập Excel hàng loạt) cũng tạo `ResidentApartmentRelation` nhưng CHƯA wire — để dành
  vì import hàng loạt là luồng khác, cân nhắc riêng.
  Backfill `community:backfill-membership-grants` (`--dry-run`/`--rollback`, mẫu
  `BackfillProjectFollows`): mỗi `community_group_members` hiện có → 1 grant
  (`is_default=true` → `system_enrollment`, còn lại → `manual_join`, `source_id=null`
  cho cả hai vì dữ liệu cũ không biết quan hệ cụ thể nào).
  **Kiểm chứng R2 (bắt buộc theo `COMMUNITY_RISK_ROLLBACK.md`)**: test tài khoản 2 căn
  2 dự án — gỡ quan hệ dự án A chỉ mất quyền nhóm A, nhóm B nguyên vẹn; test 2 grant
  cùng membership (2 căn cùng dự án, và riêng resident_relation+manual_join) — thu hồi
  1 grant giữ nguyên membership, thu hồi grant CUỐI CÙNG mới mất — cả hai đều PASS.
  9 test mới `CommunityMembershipGrantsTest`.
- **Chưa làm**: GĐ1 nền (CommunityAccessService, capability resolver hợp nhất, error
  code, idempotency middleware), GĐ5 content/feed bootstrap (kể cả
  `GET resident/community/bootstrap` — ưu tiên cao nhất theo kế hoạch, **chưa làm** vì
  hết ngân sách phiên này sau khi làm chắc GĐ3), GĐ6 ảnh, **GĐ7 tách bình luận** (vẫn
  đúng thứ tự CUỐI CÙNG — điều kiện tiên quyết seed khối lượng lớn chưa làm), GĐ8 kiểm
  duyệt tổng quát hơn B6, GĐ9 scale. `community_group_verification_history` (tạo từ
  GĐ2) vẫn chưa có service dùng tới.
- 24 test mới cho Community Domain tính tới nay (`CommunityGroupTypeTest`,
  `CommunityGroupHierarchyTest`, `ProjectFollowTest`, `CommunityMembershipGrantsTest`).
  `php artisan test` (qua `vendor/bin/phpunit` trực tiếp, `php artisan test` tự spawn
  subprocess bị giới hạn `memory_limit=128M` mặc định của Herd và OOM giữa chừng — chạy
  `php -d memory_limit=1024M vendor/bin/phpunit` thay thế): **146/147 pass** (138 cũ +
  9 mới; 1 lỗi KHÔNG liên quan — `ScreenTelemetryTest::test_tong_hop_theo_ngay_...`
  phụ thuộc giờ đồng hồ thật lúc chạy test, `now()` UTC trừ 2 giờ đôi khi lệch sang
  ngày UTC trước trong khi lệnh tổng hợp gộp theo ngày — có từ trước, không đụng file
  nào của phiên này, không tái tạo được bằng cách sửa code Community).
...
| **WEB-UX-23** Platform Content & Project Library | 🟢 | `Sa/Pages/PlatformContentDashboard.php`,`PlatformContentCms.php`,`PublicProjectLibrary.php`, Resource `PlatformContents`,`PublicProjects`, model `PlatformContent`,`PublicProject`,`ProjectMedia` | + **Nhập dự án batdongsan** (nút "Lấy tiếp"): `Services/Projects/BdsProjectImporter` (+`enrichDetail` bóc bảng "Thông tin dự án" → `metadata_json.detail`), `config/bds.php`, migration+model `BdsImportState`, command `projects:fetch-more {--no-detail}` + `projects:export-json` + seeder `PublicProjectImportSeeder` (đồng bộ local→server). + **Địa chỉ có cấu trúc** (`ward`/`district`/`province` qua `parseAddress`, cột lọc /sa) + **toạ độ** `latitude`/`longitude` (từ trang chi tiết) + **entity Chủ đầu tư** (`developers` dedup theo slug, `developer_id` FK, `DeveloperResource` /sa nhóm "Dự án") + **ảnh dự án** (`metadata_json.images`/`cover_image`, ImageColumn — ⚠️ ảnh CÓ watermark `_wm`, bản sạch HTTP 530) + **hiển thị toạ độ/detail** trên form+bảng (link Google Maps) + **địa chỉ cũ↔mới 2025** (badge confidence, cột "Tỉnh mới") + **"Tìm ảnh & thông tin" chính thống** (`config/enrichment.php` provider mock/google_cse/serpapi, `ProjectEnrichmentService`, action Edit → `metadata_json.official_*`) + **Thư viện ảnh** (`project_media` +source/is_cover/is_watermarked, `ProjectMediaSync`, command `projects:sync-media`, RelationManager "Thư viện ảnh", `projects:enrich-missing` backfill). Đã thu ~2200+ dự án, ~21k ảnh (batdongsan watermark). ⚠️ Cloudflare chặn Guzzle (OpenSSL); transport `curl` Schannel qua được — Linux prod dùng vòng export→seed. Xem `docs/dev/02_features/PUBLIC-PROJECT-IMPORT.md` |
