...
- **Copy code + card (restyle 2026-07-27):** khối code render thành CARD nền xám nhạt (`.docs-code`): header (tên ngôn ngữ + nút Copy) + gutter số dòng (`user-select:none`) + code tô màu cú pháp. **Syntax highlight server-side** qua `scrivo/highlight.php` (`Highlight\Highlighter`, đã có sẵn trong vendor — không thêm package/CDN/JS, chạy cho cả guest) trong `DocsMarkdown::styleCodeBlocks()`. Copy đọc `data-code` (đúng source, không kèm số dòng). Block không ngôn ngữ → card + số dòng, không tô màu. Theme hljs light + `@media (prefers-color-scheme: dark)`. `overflow-x:auto` khi code dài.
