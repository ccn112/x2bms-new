{{-- WEB-UX-00 · Header left: sidebar toggle (hamburger) + page title + global search. --}}
<div class="flex items-center gap-3" x-data>
    ...
    <button
        type="button"
        x-on:keydown.window.cmd.k.prevent="$el.click()"
        x-on:keydown.window.ctrl.k.prevent="$el.click()"
        x-on:click="$dispatch('open-global-search')"
        class="ml-2 flex h-10 w-[20rem] max-w-[36vw] items-center gap-2.5 rounded-xl border border-gray-200 bg-gray-50 px-3.5 text-sm text-gray-400 transition hover:bg-white hover:shadow-sm dark:border-white/10 dark:bg-white/5"
    >
        ...
        <span class="flex-1 truncate text-left">Tìm kiếm…</span>
