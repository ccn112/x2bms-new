<x-filament-panels::page>
    <x-x2.action-bar :title="'Căn hộ '.$apartment->code" :subtitle="($apartment->floor?->name ?? '').' · '.number_format((float) $apartment->area_sqm, 0).' m² · '.$apartment->building?->name">
        <a href="{{ url('/admin/apartments') }}" class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50">← Danh sách</a>
        <a href="{{ url('/fila/apartments/'.$apartment->id.'/edit') }}" class="rounded-lg bg-x2-primary px-3 py-1.5 text-sm font-medium text-white hover:bg-x2-primary-600">Chỉnh sửa</a>
    </x-x2.action-bar>
