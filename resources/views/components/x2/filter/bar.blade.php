@props([
    ...
    'advancedClick' => null, // Alpine expression to toggle advanced filters (e.g. "adv = true")
])

{{-- DS-01 business filter toolbar — sits directly above the table/list and only
     affects rows (never the context-wide KPIs). Layout:
       [ inline selects… ] [ search ......... ] [ Bộ lọc nâng cao (n) ]   [ trailing ]
     Slots: `inline` (business selects), `savedView`, `search`, `trailing`.
     Active-filter chips render below via <x-x2.filter.chip>. --}}
<div {{ $attributes->class(['flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm']) }}>
    ...
    @isset($inline)
        {{-- Inline business selects hide on mobile (<md); mobile filters live in the drawer/bottom sheet. --}}
        <div class="hidden flex-wrap items-center gap-2 md:flex">{{ $inline }}</div>
    @endisset
    ...
    <button type="button"
        @if ($advancedClick) x-on:click="{{ $advancedClick }}"
        @elseif ($advancedWire) wire:click="{{ $advancedWire }}" @endif
